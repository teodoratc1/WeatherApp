...
import { useFavoriteCities } from "@/components/favcities";
...
const Profile = () => {
  ...
  return (
    <div className="lg:pt-10 lg:max-w-[90%] mx-auto max-w-[85%]">
      <h1 className="text-[36px] font-[900]">Profilul meu</h1>
      <div className="grid md:grid-cols-2 gap-7 ">
        ...
        <ul>
          {favoriteCities.map((cityData, index) => {
            ...
            const formattedDate = formatter.format(cityData.time);
            return (
              <div key={index}>
                <div className="bg-[#1f8ecc] border text-white px-4 py-3 rounded grid grid-cols-2 shadow-lg relative">
                  <div className="flex flex-col p-2">
                    <h3 className="text-[22px] font-semibold">
                      {cityData.city}
                    </h3>
                    <div className="ml-3 text-[20px]">
                      Temperatura : {cityData.temperature}°C
                    </div>
                    <div className="mb-2">
                      Umiditatea:{" "}
                      {cityData.relativeHumidity !== undefined
